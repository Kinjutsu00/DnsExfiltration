...
From these payloads the malicious DNS script will be able to create and fill the exfiltrated file: when the packet number is "0" it knows that it has to create a file with the name equal to "fileName", and then just append data (to the current working file) from the next packets, untill a new packet with "0" packet number is received.
...
4) To have a try, run on the DNS server "composer.py" ( **:~$ python composer.py** ) and then run (or make the victim runs it) "exfiltrator.py" ( **:~$ python exfiltrator.py** ) script on the victim machine.
5) Wait and see 
